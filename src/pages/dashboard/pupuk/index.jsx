...
const Index = () => {
    ...
    const columns = [
        ...
        },
        {
            ...
            key: "kemasan",
            sorter: (a, b) => { },
            sortOrder: currentSort && currentSort.index == 'kemasan' ? currentSort.order : undefined,
        },
        {
            title: 'Action',
            align: 'center',
            render: (text, record, index) => {
                return (
                    <div className='d-flex gap-2 justify-content-center'>
                        ...
                        <button type='button' className='btn btn-danger btn-sm' onClick={() => handleDelete(record.id)} data-bs-toggle="popover" data-bs-trigger="hover" data-bs-content='Delete Pupuk'><FontAwesomeIcon icon={faTrashAlt} fixedWidth /></button>
                    </div>
                )
            }
        }
    ...
    const getData = async () => {
        ...
        await axios({
            method: 'GET',
            url: `${process.env.NEXT_PUBLIC_RESTFUL_API != undefined ? process.env.NEXT_PUBLIC_RESTFUL_API : ''}/pupuk?limit=${pageSize}&offset=${(currentPage - 1) * pageSize}${Object.keys(currentSort).length > 0 ? '&order=' + order : ''}${search != '' ? '&search=' + search : ''}${jenis !== '' ? `&jenis=${jenis}` : ''}${pengedarId !== '' ? `&pengedarId=${pengedarId}` : ''}`,
        }).then((res) => {
    ...
    const handleDelete = (id) => {
        Swal.fire({
            ...
            if (res.isConfirmed) {
                await axios({
                    method: 'DELETE',
                    url: `${process.env.NEXT_PUBLIC_RESTFUL_API != undefined ? process.env.NEXT_PUBLIC_RESTFUL_API : ''}/pupuk/${id}`
                }).then((res) => {
                    if (res.data?.id) {
                        getData()
                        Swal.fire('Success', 'Pupuk deleted successfully!', 'success')
                    }
                }).catch((err) => {
                    console.error(err)
                })
...
Index.getLayout = function getLayout(page) {
    return (
        <DashboardLayout header='Pupuk List' breadCrumb={[{ label: 'Dashboard', link: '/dashboard' }, { label: 'Pupuk', link: '/dashboard/pupuk' }, { label: 'Pupuk List', link: '/dashboard/pupuk' }]} breadCrumbRightContent={(
            <div className='d-flex justify-content-end gap-1'>
                <Link href='/dashboard/pupuk/create' className='btn btn-primary rounded-xl'>
